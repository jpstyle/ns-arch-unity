...
class StringMsgChannel(SideChannel):
    ...
    def send_string(self, utterances, dem_refs):
        assert isinstance(utterances, list) and isinstance(dem_refs, list)
        assert len(utterances) == len(dem_refs)

        msg = OutgoingMessage()
        for utt, dem_refs_for_utt in zip(utterances, dem_refs):
            # Utterance content as string
            msg.write_string(utt)

            # Write any demonstrative references
            for (start, end), ref in dem_refs_for_utt.items():
